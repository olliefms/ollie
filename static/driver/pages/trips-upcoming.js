...
export async function renderUpcomingPane(container) {
  ...
  try {
    const data = await apiFetch('/trips?tab=upcoming');
    container.replaceChildren();
    const items = (data && data.items) || [];
    if (items.length === 0) {
      const empty = document.createElement('div');
      empty.className = 'trips-empty';
      empty.textContent = 'No upcoming trips';
      container.appendChild(empty);
      return;
    }
    items.forEach(t => container.appendChild(renderTripCard(t, 'upcoming')));
  } catch (err) {
    if (err.status === 401) {
      clearAuth();
