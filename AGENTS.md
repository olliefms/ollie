...
src/
  ...
  main.rs         — wiring only: config, db, store, ai, geocoding/routing clients, pipelines, bind
  startup.rs      — serve(): what runs behind the accept loop (indices, embedding backfill,
                    pipeline recovery) + the `ollie healthcheck` probe. New startup work goes
                    HERE, not in main — see the #404 invariant below
...
**A worker task must never be allowed to panic.** The receivers live only inside the worker
tasks, so a panic that unwinds a worker drops its receiver — and `PIPELINE_WORKERS` defaults
to **1**, which makes that the last one. The channel then closes for the lifetime of the
process: every later `enqueue` fails with `sending into a closed channel` (async-channel's
...
`GET /healthz` reports each pool's live-worker count and returns 503 when one is gone.
...
The integration tests in `tests/it/integration_test.rs` do not call Ollama. The pipeline receiver is kept alive but workers are never polled. All upload tests check HTTP response codes only, not AI output.
...
| `OLLIE_MAINTENANCE_INTERVAL_SECS` | No | `21600` (6 h) | Gap between LanceDB compaction passes (#403). `0` disables the scheduler; the `compact_datasets` MCP tool still works. |
| `OLLIE_MAINTENANCE_TARGET_ROWS_PER_FRAGMENT` | No | `1048576` | Compaction target — fragments smaller than this are compaction candidates. |
...
# Integration tests only (tests/it/*.rs are modules of the single `it` binary;
# tests/isolated/*.rs hold the env-var-mutating tests)
cargo test --test it

# One former test file, e.g. tests/it/tonu_test.rs
cargo test --test it tonu_test::
...
- **The fleet SPA's `?v=` stamps do NOT cache-bust its module graph — `Cache-Control: no-cache` on the fleet `ServeDir` is what keeps it correct.** `static/fleet/index.html` stamps only `base.css`, `components.css`, and `app.js`, and a query string is dropped when resolving `app.js`'s relative `import` specifiers — so every file under `static/fleet/{pages,components,utils}/` is requested unversioned. Fleet has no service worker, and `ServeDir` sets `Last-Modified` but no `Cache-Control`, which leaves browsers on heuristic freshness. Before the fix, a deploy could pair a fresh `router.js` with a cached `utils/dom.js`. — Why: this is invisible in review and in tests-on-a-fresh-profile; only a returning browser with a warm cache reproduces it, and the symptom (filters silently not working) looks exactly like the application bug you just fixed. — How to apply: never remove the `SetResponseHeaderLayer` on `fleet_static` in `src/api/mod.rs`; `tests/it/fleet_static_cache_test.rs` pins it, including that revalidation still returns a bodyless 304. If you add a *new* static surface, give it the same layer or a service worker — don't rely on `?v=` stamps alone. Driver is exempt: its SW owns that cache. #291 tracks replacing manual stamps entirely.
...
- **Two parallel execution tracks sharing one test file will conflict at merge.** When two orchestrator sessions both add tests to `tests/it/integration_test.rs`, the merge will conflict at the same insertion point. Resolve by keeping both blocks in full; don't drop either track's tests.
...
- **Document preview must MIME-branch, not sandbox-everything-in-one-iframe.** v1.14.x doc preview originally used a single `<iframe sandbox="">` for every previewable MIME. Empirical Playwright testing in v1.14.x (#184) showed Chrome's PDF viewer fails under EVERY sandbox value — `''`, `'allow-same-origin'`, `'allow-scripts'`, and combinations all break blob-PDF rendering. The fix is to branch by type and use the right element per type: `application/pdf` → `<iframe>` with no sandbox attribute (PDFs cannot execute scripts, so origin inheritance is harmless); `image/*` → `<img>` (no iframe at all); text (`text/*` plus `application/json`, `application/xml`, `*javascript*` — mirroring `extract_content()` in `src/ai/extract.rs`, widened from `text/plain` alone in #407) → `<pre>` with `.textContent` (XSS-safe by definition); `text/html` → drop from preview, force download (the only previewable type that can execute scripts). This supersedes the earlier "always carry `sandbox=''`" rule, which assumed sandbox was free — it isn't, it breaks the PDF viewer. — Why: any single-iframe approach picks between "broken PDFs" and "XSS surface"; MIME-branching gets both right. — How to apply: in any new doc preview path, switch on MIME at the top and choose the element; reserve sandboxed iframes for types that can execute scripts AND that we actually want to render inline (currently: none). Note the `text/html` carve-out is why the viewer predicate can't be a *pure* mirror of `extract_content()` — the pipeline asks "can we extract text to summarize", the viewer asks "is this safe in the DOM", and `text/html` is the one MIME where those answers differ; match on the bare type so a `; charset=` parameter can't smuggle it past the exclusion.
...
- **Never put a server→client MCP request on the `tools/call` path — rmcp delivers them only on the standalone GET SSE stream.** rmcp's `resolve_outbound_channel` routes every `ServerJsonRpcMessage::Request` to the session's *common* channel (the stream a client opens with `GET /fleet/mcp`), never onto the POST response stream that is carrying the tool call. If the client never opened that GET stream, the send error is swallowed inside rmcp's `CachedTx` and the request simply vanishes; `peer.elicit()` applies no timeout, so the handler awaits a reply that can never arrive. This is what hung `cancel_trip` / `cancel_load` in v2.5.1 for any client that declared the `elicitation` capability — the await sat ahead of the DB write, so the record was left untouched and the failure looked like a dead session rather than a bad round-trip. Destructive-op confirmation now relies solely on the `destructiveHint` annotation, which is where MCP puts that responsibility. — Why: capability declaration proves the client *understands* elicitation, not that the transport can *deliver* it; the two are independent and only the second one matters. — How to apply: server→client **notifications** are fine. Before adding any server→client **request** (elicitation, sampling, roots) inside a tool handler, confirm it cannot block the reply, and give it a bounded timeout regardless. Any tool whose confirmation matters must appear in `annotations_for`'s destructive list.

- **MCP integration tests must declare the client capabilities the server branches on — `capabilities: {}` silently skips those paths.** Every MCP test in `tests/it/integration_test.rs` opened its session with empty capabilities, so the elicitation branch above (`if peer.supported_elicitation_modes().is_empty() { return None }`) short-circuited in *all* of them and the production hang was invisible to a green suite. A one-shot `curl` against a fresh session doesn't reproduce it either. — Why: a server that adapts to client capabilities has one code path per capability combination; a test suite that only ever declares `{}` covers exactly one of them, and it's the degenerate one. — How to apply: use `mcp_session_with_caps` and declare the capability whenever the behavior under test depends on it. For anything that could hang, drive several tool calls on one long-lived session first and wrap the call in `tokio::time::timeout` so a regression *fails* instead of hanging the suite.

- **rmcp expires an idle MCP session after 5 minutes by default — that is far shorter than a conversation, and the expiry is invisible.** `SessionConfig::keep_alive` defaults to `Some(5 min)` and is armed by *session events*, not by an open SSE stream, so a client that goes quiet — reading, thinking, calling another tool — comes back to a session the server already tore down; every request on it then fails and the client reports "MCP server disconnected" with no way to re-dial from inside the running session. rmcp logs the expiry as `WorkerQuitReason::IdleTimeout` at **`debug` on its own target**, so under `RUST_LOG=ollie=info` neither side sees an error: our logs are clean, the connector shows nothing, `/version` still answers 200, and the drop looks like a network or auth fault. `mcp_session_config()` pins it to the OAuth access-token TTL (8 h) instead. — Why: rmcp's default is sized as a zombie-reaper for sessions whose connection died, not as a policy for how long a *live* agent may pause; the two are different questions and only one of them is ours to answer. — How to apply: never construct `LocalSessionManager::default()` directly — go through `mcp_session_config()`, and re-check `keep_alive` (and whether the default changed) on every rmcp bump. When a client reports a disconnect that the server's logs know nothing about, look for a session-lifetime knob before suspecting the network.

- **Denormalized status needs release edges in the state machine, and a "holds it" predicate distinct from "references it".** `LoadStatus::can_transition_to` was forward-only, so `unassign`/`undispatch`'s attempt to walk a load back down (`Assigned → Planned`, `Dispatched → Assigned`) was rejected — and the call sites discarded the error with `let _ =`, leaving loads claiming `assigned` with no trip holding them. Two separate bugs stacked: the missing edges, and `count_active_trips_for_load` counting `planned` trips as active (so the just-released trip kept its own load pinned). `count_load_holding_trips` (assigned/dispatched/in_transit) now drives the status decision; the wider count still guards deletes, where a merely-referencing planned trip *should* block. — Why: a denormalized field mirroring a child's state must be able to move in both directions the child can, and "still references the parent" is a genuinely different question from "currently holds the parent". — How to apply: when adding a forward cascade, add the inverse edge in the same commit. Never swallow a `transition_*` error with `let _ =` in a cascade — log it (see `demote_released_load`), or the next stale-status bug will also be invisible.

- **Never use native `confirm()` / `prompt()` / `alert()` in the SPA — they block the renderer's main thread.** A `window.prompt` in `load-detail.js`'s Cancel handler was misdiagnosed in production as a hung network request: while the modal is up the tab cannot repaint, run injected script, or be closed programmatically, so a routine confirmation is indistinguishable from a dead renderer. `components/confirm.js` provides promise-based replacements (`confirmAction`, `confirmDelete`, `confirmTyped`, `promptText`, `promptFields`, `chooseOption`) that leave the event loop free; `confirmDelete` is async, so callers must `await` it. — Why: the freeze is a property of the native dialog, not of the request behind it, which makes it the wrong thing to debug and the easy thing to blame on the backend. — How to apply: no new native dialogs; grep for `confirm(`/`prompt(`/`alert(` after any page change. Pair every action button with `withPending` (spinner + disabled row) and rely on `apiFetch`'s `DEFAULT_TIMEOUT_MS` deadline; pass `timeoutMs: 0` only for uploads and downloads, whose duration legitimately scales with the payload.

- **Silently dropping an unknown request field is worse than rejecting it.** `update_load{status, cancellation_reason}` returned 200 with a normally-shaped, entirely unchanged record, because `UpdateLoadRequest` has no such fields and serde ignores unknown keys by default — any caller reads that as "applied". `unsettable_load_field` now names the owning tool instead (`use cancel_load`). — Why: a success response is a claim; a request-shaped struct plus serde's default makes that claim silently false for every field the struct doesn't model. — How to apply: for any `serde_json::from_value::<Req>(args)` in a tool handler, decide explicitly what happens to plausible-but-unmodelled fields — reject with a message pointing at the right tool, or model them. Same rule for arg *names*: `handle_tool_call` normalizes `id` ↔ `<entity>_id` via `id_arg_alias` so the two halves of one operation can't disagree, and `explain_arg_error` attaches the tool name, the accepted spellings, and the keys actually received.

- **Backlog-proportional startup work belongs behind the accept loop, in `startup::serve` — not ahead of it.** `requeue_stale` sends every stale blob into the pipeline's `bounded(256)` channel; past 256 a send only completes when a worker takes a job, so the requeue loop inherits the *drain* time. Running it before `TcpListener::bind` turned a 768-blob backlog into a projected 8.5-hour cold start where the port refused connections outright — and with no `HEALTHCHECK` in the image, `docker ps` said `Up` the whole time, so orchestrators, uptime checks and `depends_on` all believed the service was fine (#404). Index creation and the facility embedding backfill moved behind it too. — Why: "start the workers, then feed them, then serve" reads like correct startup ordering, and the coupling that makes it fatal (a bounded channel turning an enqueue loop into a drain loop) is invisible at the call site. It only reproduces with a backlog, which exists precisely when a restart just happened — so the failure compounds with itself. — How to apply: put any new backlog-proportional startup task inside the `tokio::spawn` in `src/startup.rs`, never between `main`'s bind and the `startup::serve` call; `tests/isolated/startup_recovery_test.rs` pins it by driving `startup::serve` on a real socket while `requeue_stale` is wedged on a full channel. Summaries, embeddings and geocodes are all eventually-consistent — no *read* path needs them present to answer, which is why a recovery failure now logs instead of aborting startup. The *write* paths are the catch: `ingest_blob`, driver document upload and `resummarize_blob` all send into the same saturated channel, so once the API serves during a drain they would block a request for hours and lose the job outright if the client times out mid-send. They go through `pipeline::enqueue`, which `try_send`s and only detaches when the channel is genuinely full — never through a bare `.send().await` in a handler. Two things still legitimately precede the bind and can't move (the router needs them): `Config::from_env` and `DbClient::new`, whose per-table `add_columns` migrations *are* data-proportional — if a migration ever gets slow, that's the next thing to make async, not something this rule already covers. `ollie healthcheck` (wired to the image's `HEALTHCHECK`) probes `GET /version`, so it proves the listener is *answering* — it is deliberately not a deep readiness check, since a pending backlog does not make the API unready. — The same drain-time coupling applies *between* the queues, which is why `requeue_stale`'s three drains run under one `tokio::try_join!` and must stay that way. Sequentially, each queue waits out the ones ahead of it, so a few hundred stale blobs hold facility geocoding and load routing for hours — and #406 widened that head of the line by adding retryable failures to it. The queues feed independent worker pools and share no state; only the two *pipeline* drains are ordered, because the suggestions set is derived from the blob sweep's ids and both feed the same channel. `test_a_wedged_blob_queue_does_not_hold_geocoding_and_routing` pins it by wedging the blob channel and requiring the geocode queue to fill anyway.

- **A rollup cascade over child records must compare against *intent*, not one exact status — and it needs a repair path for the rows that already missed it.** `cascade_final_stop_delivered` gated the load on `trips.iter().all(|t| t.status == Delivered)`. Two statuses fail that equality without meaning the load is unfinished: a `Cancelled` sibling (a superseded pre-assignment — a dead record that should not count at all) and a `Completed` sibling (`Delivered → Completed` is the *normal* end state, so on any relay load, completing leg 1 permanently blocked leg 2 from ever cascading). Either one stranded the load in `in_transit` forever: `invoice` runs only from `delivered` and `settle` only from `invoiced`, so the whole billing chain is out of reach, no `deliver` endpoint exists, and `cancel` is semantically wrong for freight that physically delivered — load 4819063 needed the container stopped and a one-off binary run against the LanceDB volume (#395). `models::trip::load_trips_all_delivered` is now the single predicate (filter out `Cancelled | Tonu`, accept `Delivered | Completed`, and return `false` on an empty survivor set so `all()` can't be vacuously true), and `load_doctor`'s `load.status_matches_trips` check + `advance_load_to_delivered` fix is the supported repair. — Why: the equality reads as obviously right because `Delivered` is the status the cascade itself just set; the statuses that break it are the ones the *rest* of the lifecycle sets, in a different file, after the cascade has run. Nothing in the cascade's neighbourhood hints that its own output is a transient state. — How to apply: any `all(|child| child.status == X)` rollup needs (1) dead/cancelled/tonu'd children filtered out, not counted, (2) every status reachable *from* X accepted alongside it, (3) an explicit non-empty guard, and (4) a doctor check that can walk a parent forward if the cascade ever misses. Never leave a denormalized parent status with a forward-only cascade and no manual override — the failure mode is silent and unrecoverable through the API. Same rule as the release-edge lesson above: cascades that only ever run once are the ones that strand data. — The repair itself needs the same care in reverse: `LoadStatus::Delivered` has no reverse edge, so a doctor that auto-advances on the trip predicate alone would be equally unrecoverable when it's wrong. Deliver leg 1 of a relay, then cancel the still-`Planned` leg 2, and every *live* trip has delivered while the load's consignee stop was never reached — auto-advancing there bills the customer for freight that never arrived. `uncovered_delivery_stops` corroborates by matching each load `Delivery` stop to a delivery stop on some surviving trip, by facility, and downgrades the fix to reported-with-conflicts when one is unmatched. **Corroborating on the load's own `actual_depart` values instead is the trap** — that data only exists when `load_stop_index` is set, and `apply_trip_create` populates it *only* for a trip created with no explicit stops that derives every stop from the load, which a relay leg by definition cannot do. A signal keyed on actuals is therefore silent on exactly the multi-trip loads it is meant to guard, and noisy on healthy single-trip ones. Trip stops carry `facility_id` + `stop_type` regardless, so they are the signal that is actually there. A trip stop with no `facility_id` is unmatchable and might be the covering one — absence of signal, not evidence, so the fix stays applyable.
...
- **LanceDB reclaims nothing on its own — every new table must be listed in `DbClient::tables()` or it silently leaks file descriptors forever.** Lance is append-only, and **two** counts grow unbounded — confusing them sends you at the wrong half of the fix. *Fragments* track live rows: every insert lands in its own fragment and nothing merges them, so a table settles at one file per row, and an update does **not** raise this (the `merge_insert` upsert writes a replacement fragment and tombstones the one it supersedes, netting zero). *Data files and version manifests* track writes ever made: every write leaves both, and nothing reclaims them. A single-truck fleet showed exactly that shape — 2,069 fragments for 2,069 blob rows, but 2,940 data files and 2,813 manifests, 30,371 files total under `/data/lancedb` against a container default `ulimit -n` of 1024, at which point reads fail intermittently with `Too many open files` and *which* read fails first is arbitrary (#403). Measured locally: one row plus ten updates = 1 fragment, 11 data files, 12 manifests. Compaction reclaims the first count, pruning old versions reclaims the second — a pass must do both or it only half-works. `services::maintenance` prunes and compacts every dataset on a schedule (in that order — see below); `compact_datasets` (dry-run by default) is the on-demand path and the fragmentation report. — Why: the ratio is invisible from every surface that matters — row counts look tiny, queries look fine, and the failure arrives as an unrelated-looking IO error on whichever table you touched next. It needs no bulk import to get there, only time. — **The pass must prune BEFORE it compacts**, and `tests/it/maintenance_test.rs::test_pass_does_not_delete_files_an_in_flight_read_is_using` pins it. Compaction supersedes the current version without deleting anything; the prune is the only step that deletes. Run it second and it removes the files compaction just orphaned — lance classifies a data file referenced solely by manifests it is deleting as *verified* and deletes it unconditionally, and lance's 7-day threshold does not cover it (that guards files **no** manifest references, i.e. in-flight writes). Any request already scanning the pre-compaction snapshot then dies with `No such file or directory`, which is the same intermittent read failure the compaction exists to remove. Reproduced deterministically with zero retention; in production it needs only a table whose last write predates the retention window — routine for terminals/trucks/drivers, and true of *every* table on the first pass after a restart. Pruning first leaves the live snapshot referenced and safe; what compaction orphans is reclaimed a pass later, when no reader can still hold it. — How to apply: (1) any new table added to `DbClient` goes into `DbClient::tables()` in the same commit — `test_tables_covers_every_dataset` pins this against the connection's own `table_names()`, so an omission fails a test rather than an instance six months later. (2) Compact through the *same* `Table` handle the request path reads from; `optimize` refreshes the handle it is called on, so a separately-opened table leaves every existing reader pinned to a manifest whose files the prune just deleted. (3) **Never compact externally with `pylance`** — `pylance` 9.0.0 against a `lance-io 6.0.0` store silently upgraded four tables from `data_storage_version` 2.0 to 2.1 in testing, which the server may not read back. In-process compaction exists precisely so the version can't skew. (4) Deployments need `ulimits: nofile` raised (65536 in the shipped `docker-compose.yml`); compaction bounds the file count but the headroom is what survives a heavy import between passes. Note this pairs badly with #404: the fd exhaustion forces a restart, and the restart is what costs hours.

- **A trip's stop list must end at the last position the truck actually reached — and that rule is self-satisfying once a waypoint is already there.** ORS routes waypoint to waypoint, so any recompute that skips where the truck really got to draws the route it *would* have taken: a load diverted 200 miles out and turned back toward the shipper drove 400, but recomputed as `stop0 -> shipper` it claims whatever mileage that pair happens to produce, and the number still looks plausible enough that nobody checks it. `tonu_trip` truncates the stop list to the last stop with an `actual_arrive` (or records a `TripStopType::Waypoint` when none was reached); `divert_trip` requires a fresh `waypoint` on every call *except* one — a trip that already ends at a reached `Waypoint`, left there by an earlier hold-only divert, needs no new divergence point, because its existing stop list already satisfies the invariant (`ends_at_reached_waypoint` in `divert`, `src/services/trip_lifecycle.rs`). The first design made `waypoint` unconditionally required, which dead-ended the "pulled over, disposition unknown" workflow: dispatch could record where the truck stopped, but never resume the trip afterward without inventing a duplicate waypoint at the same spot. — Why: mileage bugs that under-report are loud (a driver complains); mileage bugs that mis-route are silent, because a wrong-but-plausible number passes every check a human makes. — How to apply: whenever a plan changes mid-execution, ask what the router will now walk. If the answer skips a place the vehicle physically went, you need a waypoint — unless the stop list already ends at one, in which case requiring it again is the bug, not the fix.
...
- **Clear a value before attempting to recompute it, or a failed recompute leaves the stale value sticky by default.** `tonu` clears a trip's `deadhead_miles`/`loaded_miles` before recomputing; `divert` originally did not. The gap mattered because `recalculate_miles_handler` short-circuits on `already_set = deadhead_miles.is_some() && loaded_miles.is_some()` (`src/api/fleet_portal/trip_writes.rs:151`) and returns the existing summary without calling ORS again — unless the caller passes `force: true`, so the value is not wrong *forever*, only wrong by default. That default is the real hazard: mileage has no other hand-settable path, so the obvious recovery move — call recalculate — returns a cheerful 200 carrying the stale figure, with no error and no hint that nothing changed, and that figure is exactly what `compute_driver_pay` bills the per-mile driver against. `divert` was fixed to clear mileage (`update_trip_mileage(..., None, None, None, vec![])`) before attempting the recompute, exactly as `tonu` already did, so the failure state is "empty and visibly unresolved" rather than "wrong and indistinguishable from correct." — Why: "recompute" reads as safe because the common case genuinely does replace the number, and `force` reads as the safety valve — but nobody reaches for `force` on a call that returned 200. — How to apply: whenever a write path recomputes a value some other caller treats as "already set → skip," clear the value before attempting the recompute, never after. A failed attempt must leave the field empty, not leave the old answer standing in for a new one, `force` flag or not.

- **Counting records is not the same as counting the thing the fee is for.** `compute_driver_pay` charged `extra_stop_fee` per stop beyond two with no notion of stop kind, so a trip that stopped for fuel paid the driver an extra-stop fee for fueling — live, for as long as fuel stops have been modelled. The fix is `TripStopType::is_service_stop()` (true for `Pickup`, `Delivery`, `Relay`, `EmptyMove`), applied to the extra-stop count only; detention still iterates every stop, because a driver held three hours at a waypoint is owed for it. This is a real pay change, not a no-op refactor: unsettled trips carrying `Fuel`, `Origin`, `Terminal`, or `Maintenance` stops compute lower `extra_stop_pay` once this ships, while settled trips are untouched because `driver_pay_snapshot` freezes them. — Why: the defect is invisible until a new stop *kind* is added for a non-work reason, at which point it looks like a new bug rather than an old one. — How to apply: when a fee is computed from `collection.len()`, name what the fee is actually for and filter to that before adding a member that does not qualify.
...
- **In a test suite where a dependency is deliberately unavailable, an assertion about that dependency's output passes for the wrong reason.** Three separate times on this branch a test passed while proving nothing: a `trip_doctor` test discarded the `tonu_trip` response it was meant to check, so it stayed green before TONU's release path even existed; an `assert_ne!(load["status"], "tonu")` passed for any wrong value, not only the right one; and `tests/it/tonu_test.rs`'s `assert!(trip["loaded_miles"].is_null(), "a TONU trip has zero loaded miles")` is true under the suite's `RoutingClient::new("")` whether or not the mileage code ran at all — with ORS disabled, `loaded_miles` is never set to `Some` by any path, so the branch's headline invariant (the whole routed figure goes to deadhead, loaded stays `None`) had no positive coverage, and a swapped-argument regression that put mileage in the wrong field would still have shipped green. The fix in every case was the same: stop trying to exercise the missing dependency, and instead seed the value directly through the db layer (`db.insert_trip` with `deadhead_miles`/`loaded_miles` set by hand, as `terminals_pay_settlement_test.rs`'s pay tests now do, or `db.update_trip_mileage` on an existing record, as the divert mileage test does) or assert on the value actually returned instead of discarding it. — Why: a suite that deliberately runs "with no Ollama" / "with no ORS" (see Test Patterns above) makes every path gated on that dependency's output look tested when it is only ever reached *through* it — the assertion and the missing dependency cancel out, and the test file still reads as coverage. — How to apply: before trusting a green test near a mileage-, pay-, or status-dependent change, check whether it exercises the gated code path or only asserts on a value the suite's own config already forces (empty, unset, unreachable). If the dependency is stubbed out, seed the state directly through the db layer instead of asserting on what the stub would have produced.

- **`visibility` is the driver-exposure boundary for a blob — not which record it is attached to.** The operational shorthand after the 2026-07-08 leak was *"attach the rate con to the load, never to the trip."* That describes a boundary the code does not enforce. `GET /driver/api/v1/trips/{id}/documents` selects on the **`trip:{id}` tag** and then filters on `visibility == Driver || uploaded_by == <this driver>`; `GET .../documents/{blob_id}/content` re-checks both conditions independently, so the gate is not list-only. `BlobVisibility` defaults to `Private` and is immutable after upload (`UpdateBlobRequest` carries name/tags/summary only). Driver uploads are stamped `Driver` unconditionally (`driver_portal/documents.rs`); the presigned fleet path and the MCP `upload_blob` tool are always `Private`; the multipart `POST /fleet/api/v1/blobs` is `Driver` only when the uploader explicitly sends `visibility=driver` — #186's deliberate "visible to driver" checkbox — and `Private` otherwise. So nothing becomes driver-readable as a side effect of where it hangs; it takes an explicit act. Both halves of the shorthand are wrong in a way that matters: a `Private` blob tagged `trip:{id}` is **safe** (attaching operational documents to the trip is fine), and attaching to the load is **not protective** — a blob uploaded `visibility=driver` with a `trip:` tag is exposed regardless, and driver uploads already land on `load.blob_ids` anyway, so load association and driver visibility are independent axes (#425). — Why: the shorthand is simultaneously stricter than necessary and weaker than it sounds, and it points reviewers at the wrong field, so a real exposure (someone setting `visibility=driver` on a rate con) passes a review that only checks load-vs-trip. — How to apply: when reviewing document exposure, look at `visibility` and the `trip:` tag, never at which record the blob is associated with. `test_driver_cannot_see_private_doc`, `test_driver_sees_dispatch_uploaded_driver_visible_doc`, `test_driver_list_excludes_load_blob_without_trip_tag` and `test_driver_document_content_gate` pin all four corners; keep the endpoint doc comments in `src/api/driver_portal/documents.rs` in sync with any change to the rule.

- **The driver surface is an allowlist by construction, and that is the only thing keeping it one.** `internal_notes` (#423) exists on `LoadRecord` and `TripRecord` and must never reach `/driver/api/v1`. Nothing enforces that at the type level — it holds solely because every driver handler returns a hand-built `Driver*` struct (`DriverTripListItem`, `DriverTripDetailResponse`, `DriverTripStopSummary`, `DriverStopDetailResponse`, `DriverTripLoadSummary`) populated field-by-field, never a serialized domain model. `DriverTripLoadSummary` is four fields precisely so a load's rate and notes are unreachable from the driver's trip view. — Why: adding a field to a domain model is a one-line change that silently widens every endpoint that serializes that model wholesale; here it widens nothing, but only for as long as the convention holds, and the 2026-07-08 revenue leak is what the convention costs when it doesn't. — How to apply: never return a `LoadRecord`, `TripRecord`, `LoadListItem`, or `TripListItem` from a handler under `/driver/api/v1` — map into a `Driver*` type. When adding a dispatcher-only field to a shared model, add it to the sentinel list in `tests/it/driver_internal_notes_test.rs`, which asserts on the *serialized body* of every driver read endpoint (so a leak through a renamed or nested field still fails) and guards against a vacuous pass by first requiring the dispatcher surface to expose the same value. The driver read endpoints and their response schemas are published in `/openapi.json` (#424) as deliberately distinct types, never `$ref`s to the fleet schemas — that is what makes "can a driver see this field?" answerable without reading `data.rs`.
