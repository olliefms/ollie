...
- **Clear a value before attempting to recompute it, or a failed recompute makes the stale value permanent.** `tonu` clears a trip's `deadhead_miles`/`loaded_miles` before recomputing; `divert` originally did not. The gap mattered because `recalculate_trip_miles`'s handler short-circuits on `already_set = deadhead_miles.is_some() && loaded_miles.is_some()` (`src/api/fleet_portal/trip_writes.rs:123`) and returns the existing summary without calling ORS again — mileage has no other hand-settable path — and that stale figure is exactly what `compute_driver_pay` bills the per-mile driver against. So a stale number surviving an ORS outage at divert time is not wrong for one read; it is wrong forever, because every later recompute attempt sees "already set" and stops, and the driver gets paid against a route he never drove. `divert` was fixed to clear mileage (`update_trip_mileage(..., None, None, None, vec![])`) before attempting the recompute, exactly as `tonu` already did. — Why: "recompute" reads as safe because the common case genuinely does replace the number — the trap only shows up in the failure branch, where clearing vs. not-clearing is the difference between "empty and known-incomplete" and "wrong and indistinguishable from correct." — How to apply: whenever a write path recomputes a value that some other caller treats as "already set → skip," clear the value before attempting the recompute, never after. A failed attempt must leave the field empty, not leave the old answer standing in for a new one.
