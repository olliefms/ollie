...
- **Nothing that scales with data volume may run ahead of the listener bind in `main`.** `requeue_stale` sends every stale blob into the pipeline's `bounded(256)` channel; past 256 a send only completes when a worker takes a job, so the requeue loop inherits the *drain* time. Running it before `TcpListener::bind` turned a 768-blob backlog into a projected 8.5-hour cold start where the port refused connections outright — and with no `HEALTHCHECK` in the image, `docker ps` said `Up` the whole time, so orchestrators, uptime checks and `depends_on` all believed the service was fine (#404). Index creation and the facility embedding backfill moved behind the bind with it. — Why: "start the workers, then feed them, then serve" reads like correct startup ordering, and the coupling that makes it fatal (a bounded channel turning an enqueue loop into a drain loop) is invisible at the call site. It only reproduces with a backlog, which exists precisely when a restart just happened — so the failure compounds with itself. — How to apply: in `main`, bind and log `listening on` before any DB-volume-proportional work, then `tokio::spawn` that work; `tests/startup_recovery_test.rs` pins it by serving a real socket while `requeue_stale` is wedged on a full channel. Summaries, embeddings and geocodes are all eventually-consistent — no API path needs them present to answer. Keep `ollie healthcheck` wired to the image's `HEALTHCHECK` so "up" never means "ready" again.
