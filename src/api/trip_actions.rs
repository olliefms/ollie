...
#[utoipa::path(
    ...
    params(("id" = Uuid, Path, description = "Trip UUID")),
    responses(
        (status = 200, description = "Trip completed and resources released", body = TripRecord),
        (status = 401, description = "Unauthorized"),
        (status = 404, description = "Not found"),
...
pub async fn complete_trip(
    ...
    let trip = state.db.transition_trip_status(id, TripStatus::Completed).await?;
    ...
    events::on_trip_completed(
        &state.db, id,
        existing.driver_id, existing.truck_id, &existing.trailer_ids,
    ).await;
