...
/// The full `tools/list` catalogue. Lives in `mcp_tools.json` (embedded at
/// compile time) rather than a `json!` literal: as one macro invocation the
/// ~80 tool schemas were the largest single expression in the crate — a real
/// compile-time cost re-paid on every edit to this file — and the parsed tree
/// was rebuilt on every MCP `tools/list` round-trip. Parsed once, served from
/// a static. Schema edits go in the JSON file; `tool_catalog()` and its tests
/// consume the same data, so the advertised contract cannot drift.
fn tools_list() -> Value {
    static TOOLS: std::sync::OnceLock<Value> = std::sync::OnceLock::new();
    TOOLS
        .get_or_init(|| {
            serde_json::from_str(include_str!("mcp_tools.json"))
                .expect("mcp_tools.json is valid JSON")
        })
        .clone()
}
