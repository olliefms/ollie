...
const AUDIENCE: &str = "ollie-fleet_user";
const KID: &str = "v1";
const EXPIRY_SECS: u64 = 8 * 3600;

#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct FleetUserClaims {
    pub fleet_user_id: String,
...
impl FleetUserClaims {
    ...
    /// fleet_user's effective scopes do not grant it.
...
pub fn encode_fleet_user_jwt(id: Uuid, token_version: i64, secret: &str) -> Result<String, AppError> {
    let now = jsonwebtoken::get_current_timestamp() as usize;
    let claims = FleetUserClaims {
        fleet_user_id: id.to_string(),
        token_version,
        iss: ISSUER.into(),
        aud: AUDIENCE.into(),
...
pub fn decode_fleet_user_jwt(token: &str, secret: &str) -> Result<FleetUserClaims, AppError> {
    let mut validation = Validation::default();
    validation.set_issuer(&[ISSUER]);
    validation.set_audience(&[AUDIENCE]);
    decode::<FleetUserClaims>(token, &DecodingKey::from_secret(secret.as_bytes()), &validation)
...
mod tests {
    ...
    fn test_encode_decode_roundtrip() {
        ...
        let token = encode_fleet_user_jwt(fleet_user_id, token_version, SECRET).unwrap();
        let claims = decode_fleet_user_jwt(&token, SECRET).unwrap();
        assert_eq!(claims.fleet_user_id, fleet_user_id.to_string());
    ...
    fn test_require_scope_grants_and_denies() {
        let mut claims = FleetUserClaims {
            fleet_user_id: Uuid::new_v4().to_string(),
            token_version: 0,
            iss: ISSUER.into(),
            aud: AUDIENCE.into(),
            exp: 0,
    ...
    fn test_effective_scopes_not_serialized_into_jwt() {
        // The token must not carry authority — it only identifies the fleet_user.
        let token = encode_fleet_user_jwt(Uuid::new_v4(), 1, SECRET).unwrap();
        let decoded = decode_fleet_user_jwt(&token, SECRET).unwrap();
    ...
    fn test_wrong_secret_rejected() {
        let fleet_user_id = Uuid::new_v4();
        let token = encode_fleet_user_jwt(fleet_user_id, 1, SECRET).unwrap();
        let result = decode_fleet_user_jwt(&token, "wrong-secret-that-is-also-long-enough");
    ...
    fn test_expired_token_rejected() {
        ...
        let claims = FleetUserClaims {
            fleet_user_id: fleet_user_id.to_string(),
            token_version: 1,
            iss: ISSUER.into(),
            aud: AUDIENCE.into(),
        ...
        let result = decode_fleet_user_jwt(&token, SECRET);
        assert!(matches!(result, Err(AppError::Unauthorized)));
    }
}
