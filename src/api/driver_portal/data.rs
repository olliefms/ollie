...
pub struct DriverTripStopSummary {
    ...
    pub scheduled_arrive_utc: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub scheduled_arrive_end_utc: Option<String>,
    ...
    let stops = trip
        ...
        .map(|s| {
            ...
            let actual_depart_utc = s.actual_depart.as_deref()
                .and_then(|v| crate::models::load::naive_local_to_utc(v, s.timezone.as_deref()));
            let scheduled_arrive_utc = s.scheduled_arrive.as_deref()
                .and_then(|v| crate::models::load::naive_local_to_utc(v, s.timezone.as_deref()));
            let scheduled_arrive_end_utc = s.scheduled_arrive_end.as_deref()
                .and_then(|v| crate::models::load::naive_local_to_utc(v, s.timezone.as_deref()));
            DriverTripStopSummary {
                ...
                scheduled_arrive_utc,
                scheduled_arrive_end_utc,
...
pub async fn stop_detail(
    ...
    let actual_depart_utc = stop.actual_depart.as_deref()
        .and_then(|v| crate::models::load::naive_local_to_utc(v, stop.timezone.as_deref()));
    let scheduled_arrive_utc = stop.scheduled_arrive.as_deref()
        .and_then(|v| crate::models::load::naive_local_to_utc(v, stop.timezone.as_deref()));
    let scheduled_arrive_end_utc = stop.scheduled_arrive_end.as_deref()
        .and_then(|v| crate::models::load::naive_local_to_utc(v, stop.timezone.as_deref()));
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Stop updated. SIDE EFFECT: Recording the FINAL stop departure moves the trip to `delivered` and may AUTO-DISPATCH A DIFFERENT TRIP — the successor is the `assigned` trip on the same driver whose `previous_trip_id` is this trip. Selection is chain-only (no recency or schedule fallback): zero candidates dispatches nothing, and journals `trip.auto_dispatch_no_chain` when the driver has other assigned trips that did not chain off this one; more than one dispatches nothing and journals `trip.auto_dispatch_ambiguous`. It also declines if the candidate's truck or any trailer is bound to another active trip. When it does fire the cascade is wide: the successor trip, the driver, the truck, every trailer, and a linked `assigned` load all move to `dispatched`, though each write is best-effort and a failure is logged rather than rolled back. Only `trip.dispatched` and `load.dispatched` are journalled, with `actor: \"auto_dispatch\"`; driver/truck/trailer status changes emit no event."),
        (status = 401, description = "Unauthorized"),
