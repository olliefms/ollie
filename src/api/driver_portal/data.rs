...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Stop updated. SIDE EFFECT: Recording the FINAL stop departure moves the trip to `delivered` and may AUTO-DISPATCH A DIFFERENT TRIP — the successor is the `assigned` trip on the same driver whose `previous_trip_id` is this trip. Selection is chain-only (no recency or schedule fallback): zero candidates dispatches nothing, and more than one dispatches nothing and journals `trip.auto_dispatch_ambiguous`. It also declines if the candidate's truck or any trailer is bound to another active trip. When it does fire the cascade is wide: the successor trip, the driver, the truck, every trailer, and a linked `assigned` load all move to `dispatched`. Automatic transitions are journalled with `actor: \"auto_dispatch\"`."),
        (status = 401, description = "Unauthorized"),
