...
#[openapi(
    ...
    components(
        schemas(
            ...
            models::trip::MileageSummary,
            models::trip::DeadheadOrigin,
            models::trip::LegMiles,
...
surface. Every endpoint below except the /auth routes is described in
/openapi.json (tag: driver). The driver read schemas are published as distinct
types, never $refs to the fleet Trip/Stop schemas, because the driver surface is
a strict subset: no rate or revenue data, no load notes, no internal_notes.
