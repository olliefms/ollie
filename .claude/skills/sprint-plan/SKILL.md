...
This skill leans on `/work-issue`'s rules for the parts they share — triage, the Opus cap, the version-number prohibition, the self-merge gate. It spells out only what's different about a batched sprint: the guardrail that decides whether a sprint is even justified, and the worktree + plan + subagent execution machinery.
...
2. **Scope.** Read each in-scope issue *and its comments* (comments hold the live scope). Restate the scope and the dependency that justified batching, then wait for confirmation.
...
10. **Self-merge if clean** — same gate as `/work-issue` (blockers 0, significants 0 and addressed, CI green, tests green). Squash-merge, exit the worktree, close each issue with a verification comment. If the gate fails, leave the PR open and tell the user.
